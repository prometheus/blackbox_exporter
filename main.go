...
var (
	...
	logLevelProber = kingpin.Flag("log.prober", "Log level from probe requests. One of: [debug, info, warn, error]").Default("").String()
...
func run() int {
	...
	if *logLevelProber != "" {
		if err := allowedLevel.Set(*logLevelProber); err != nil {
			allowedLevel = nil
		}
	} else {
		allowedLevel = nil
