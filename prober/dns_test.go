...
func TestRecursiveDNSResponse(t *testing.T) {
	...
	for _, protocol := range PROTOCOLS {
		...
		for i, test := range tests {
			...
			defer cancel()
			result := ProbeDNS(testCTX, addr.String(), url.Values{}, config.Module{Timeout: time.Second, DNS: test.Probe}, registry, log.NewNopLogger())
...
func TestDNSProtocol(t *testing.T) {
	...
	for _, protocol := range PROTOCOLS {
		...
		testCTX, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		result := ProbeDNS(testCTX, net.JoinHostPort("localhost", port), url.Values{}, module, registry, log.NewNopLogger())
		if !result {
