...
func ProbeHTTP(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger log.Logger) (success bool) {
	...
	client, err := pconfig.NewClientFromConfig(httpClientConfig, "http_probe", true, false)
	if err != nil {
	...
	noServerName, err := pconfig.NewRoundTripperFromConfig(httpClientConfig, "http_probe", true, false)
	if err != nil {
